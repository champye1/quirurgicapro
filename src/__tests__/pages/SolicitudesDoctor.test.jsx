...
describe('Solicitudes (Doctor)', () => {
  ...
  it('muestra los filtros de estado', async () => {
    renderPage()
    await waitFor(() => {
      const text = document.body.textContent.toLowerCase()
      expect(
        text.includes('todas') || text.includes('pendiente') || text.includes('aceptada')
      ).toBe(true)
    })
  })

  it('muestra mensaje o lista cuando no hay solicitudes', async () => {
    renderPage()
    await waitFor(() => {
      const text = document.body.textContent.toLowerCase()
  ...
  it('el filtro de estado es interactuable', async () => {
    renderPage()
    await waitFor(() => {
      const select = screen.getByRole('combobox')
      expect(select).toBeInTheDocument()
      expect(() => fireEvent.change(select, { target: { value: 'aceptada' } })).not.toThrow()
    })
  })
})
